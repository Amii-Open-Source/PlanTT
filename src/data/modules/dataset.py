...
class MLMDataset(Dataset):
    """
    Datasets dedicated to Masked Language Models (MLMs) pre-training.
    """
    def __init__(self,
                 x: Tensor,
                 mask: Tensor) -> None:
        ...
        Args:
            ...
            mask (Tensor): binary mask indicating tokens to mask (N, 512).
        """
        super().__init__()
        self.__x: Tensor = x
        self.__mask: Tensor = mask
...
class EncoderDecoderDataset(Dataset):
    ...
    def __init__(self,
        ...
        Args:
            x (Tensor): one-hot encoded sequences (N, 5, 3000)
            masking_percentage (float): Percentage of elements replaced by an 'X' or
                                        an 'N' during masking. More precisely, the selected
                                        elements are being replaced by the [0, 0, 0, 0, 1] encoding.
            masking_span (int): number of contiguous nucleotides masked within each window.
        """
        self.__x: Tensor = x
        if masking_percentage > 0:
            self.__mask: Tensor = self.create_mask(nb_sequences=self.__x.shape[0],
                                                   sequence_length=self.__x.shape[2],
                                                   span_start_proba=masking_percentage/masking_span,
                                                   masking_span=masking_span)

            self.__get: Callable = self.__default_getter
        else:
            self.__mask: Tensor = None
    ...
    def from_path(cls,
        ...
        Args:
            path (str): path of the pickle file.
            masking_percentage (float): Feature dropout probability used during
                                        the creation of the mask. It represents the
                                        probability of a nucleotide to be replaced
                                        by an 'X' or an 'N'. More precisely, the
                                        selected elements are being replaced
                                        by the [0, 0, 0, 0, 1] encoding.
        """
        with open(path, 'rb') as file:
            _, x, _, _, _, _ = load(file)

        x = cat([x[:, :, :3000], x[:, :, 3030:]]).float() # (N, 5, 6030) -> (2N, 5, 3000)
    ...
    def create_mask(nb_sequences: int,
        ...
        """
        Generates a binary mask which coordinates that are 1s represent
        basepairs that need to be masked.

        Args:
            nb_sequences (int): number of sequences.
            sequence_length (int): length of each sequence.
            masking_percentage (float): percentage of elements to mask.
            span_start_proba (float): probability of a nucleotide to be selected
                                      as a starting point for span masking.
            masking_span (int): number of contiguous nucleotides masked within each window.

        Returns:
            Tensor: coordinates of the elements to mask (*, 2)
        """
