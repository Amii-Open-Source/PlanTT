...
class MLMTrainer(Trainer):
    ...
    def __init__(self,
        ...
        """
        Initializes the vocabulary, sets the masking probability, 
        sets the masking span, and then sets other protected attributes
        to their default values.
        
        Args:
            pad_token_id (int): id associated to [PAD] token.
            mask_token_id (int): id associated to [MASK] token.
            masking_percentage (float): expected percentage of tokens to be dynamically 
                                        mask in each sequence during training. Only the
                                        tokens that are neither [PAD], [CLS], or [SEP] will
                                        be considered for masking.
                                        Default to 0.15.
                                        
            masking_span (int): expected number of contiguous tokens mask for 
                                each masked window.
                                Default to 6.
        """
    ...
    def _update_weights(self,
        ...
        for token_ids, _ in dataloader:
            ...
            # Mask token ids
            out = self.mask_token_ids(token_ids,
                                      self.__masking_percentage,
                                      self.__masking_span,
                                      self.__pad_token_id,
                                      self.__mask_token_id)

            masked_token_ids, input_mask, padding_filter = out[0], out[1], out[2]
            ...
            # Transfer data on the device
            masked_token_ids, labels = masked_token_ids.to(device), labels.to(device)
            ...
            # Execute the forward pass and compute the loss
            loss = model(input_ids=masked_token_ids,
    ...
    def evaluate(self,
        ...
        Returns:
            float: when the method is called outside of the train method,
        ...
        with no_grad():
            ...
            for token_ids, input_mask in dataloader:
                ...
                # Set all tokens that need to be masked to [MASK] id
                bool_filter = input_mask == 0
                masked_token_ids = where(bool_filter, token_ids, self.__mask_token_id)

                # Create lables by setting to -100 all the tokens that are not masked
                labels = where(bool_filter, -100, token_ids).long()

                # Generate attention mask (0 for padding token and 1 otherwise)
                attention_mask = where(masked_token_ids == self.__pad_token_id,
                                       0, ones(masked_token_ids.shape)).long()
    ...
    def mask_token_ids(token_ids: LongTensor,
        ...
        """
        Masks a percentage of token ids that are neither associated
        to [PAD], [CLS], or [SEP] tokens.
        
        Masking is done by randomly selecting regions of k contiguous token ids.
        
        Args:
            token_ids (LongTensor): batch of sequences of token ids (BATCH SIZE, 512).
            masking_percentage (float): expected percentage of tokens (that are neither [PAD],
                                        [CLS], or [SEP]) masked in each sequence.
        ...
        Returns:
            LongTensor: masked token ids (BATCH SIZE, 512).
            LongTensor: binary valued tensor indicating tokens that are masked (BATCH SIZE, 512).
            LongTensor: binary valued tensor indicating padding tokens (BATCH SIZE, 512).
        """

        # Set 'span_start_proba' as the probability of each token to
        # be selected as a starting point in a mask span
        span_start_proba = masking_percentage/masking_span
        mask = ones(token_ids.shape)*span_start_proba

        # Zero the probability associated to the [CLS] and [SEP] tokens
        # at the start and end of each sequence respectively
...
class EncoderDecoderTrainer(Trainer):
    """
    Class encapsulating methods necessary to pre-train a 
    contrast model using one-hot encoded sequence.
    """
    def __init__(self,
                 masking_percentage: float = 0.15,
        ...
        """
        Sets the name of the loss function using the parent constructor, then sets
        other protected attributes.
        
        Args:
            masking_percentage (float): expected percentage of nucleotides to be
                                        dynamically masked in each sequence during training.
                                        Default to 0.15.
            average_masking_span (int): average masking span sampled from geometric 
                                        distribution during training epoch.
                                        Default to 6.
    ...
    def _update_weights(self,
        ...
        # Sample masking span from a geometric distribution shifted by 1 
        # and make sure it stands in the range [1, self.__max_span]
        masking_span = int(min(self.__geometric_distribution.sample().item() + 1,
                               self.__max_span))

        # Calculate the probability of a nucleotide to be selected
        # as a starting point for span masking
        ...
        for seq in dataloader:
            ...
            mask = EncoderDecoderDataset.create_mask(seq.shape[0], seq.shape[2],
                                                     span_start_proba, masking_span)
...
class SelfSupervisedTrainer(Trainer):
    ...
    def _update_weights(self,
        ...
        Executes one training epoch.
            ...
            Args:
                model (Module): PlanTT tower generating embeddings.
                device (device): device on which to send the model and its inputs.
        ...
        for seq in dataloader:
            ...
            # Execute the forward pass and compute the loss
            with autocast(device_type=device.type, dtype=float16):
                predictions = model(seq) # (BATCH SIZE, 5, 3000) -> (BATCH SIZE, EMB SIZE)
                seq = seq.reshape(seq.shape[0], -1) # (BATCH SIZE, 5, 3000) -> (BATCH SIZE, 15 000)
                loss = self.__loss_function(self.__cosine_similarity(predictions),
                                            self.__cosine_similarity(seq))
    ...
    def evaluate(self,
        ...
        Evaluates the current model using the data provided in the given dataloader.
            ...
            Args:
                model (Module): PlanTT tower generating the embeddings.
                device (device): device on which to send the model and its inputs.
                dataloader (DataLoader): dataloader containing test or validation data.
                metrics (list[Metric]): not used. Only there to match parent's function signature.
                scaler (Scaler): not used. Only there to match parent's function signature.
        ...
        with no_grad():
            ...
            for seq in dataloader:
                ...
                with autocast(device_type=device.type, dtype=float16):
                    predictions = model(seq) # (BATCH SIZE, 5, 3000) -> (BATCH SIZE, EMB SIZE)
                    seq = seq.reshape(seq.shape[0], -1) # (BATCH SIZE, 3000, 5) -> (BATCH SIZE, 15k)
                    loss = self.__loss_function(self.__cosine_similarity(predictions),
                                                self.__cosine_similarity(seq))
