...
class CNN1D(Tower):
    """
    1D Convolutional neural network (CNN) tower architecture.
    """
    def __init__(self,
                 seq_length: int,
        ...
        Args:
            ...
            feature_dropout (float): Probability of a nucleotide to be replaced by an 'X' or an 'N' during training.
                                     More precisely, the selected elements are being replaced by the [0, 0, 0, 0, 1] encoding.
            dropout (float, optional): probability of an element to be zeroed in a convolution layer. Default to 0.
        """
        if seq_length % 8 != 0:
    ...
    def forward(self, seq: Tensor) -> Tensor:
        ...
        Args:
            seq (tensor): batch of gene sequences (BATCH SIZE, 1, SEQ LENGTH, 5)

        Returns:
            tensor: embedding of the gene sequences (BATCH SIZE, SEQ LENGTH/8)
        """
        # Reshape the tensor
        seq = seq.squeeze(dim=1).permute(0, 2, 1)  # (BATCH SIZE, 1, SEQ LENGTH, 5) -> (BATCH SIZE, 5, SEQ LENGTH)
        
        # If in training, set random nucleotides to [0, 0, 0, 0, 1]
        if self.training and self.__feature_dropout > 0:
            
            # Generate indexes to dropout
            p = bernoulli(ones(seq.shape[0], seq.shape[-1])*self.__feature_dropout).nonzero().to(seq.device)
            
            # Replace the data at these positions
            seq[p[:, 0], :, p[:, 1]] = Tensor(NUC2ONEHOT_MAPPING['X']).to(seq.device)
        
        # Forward pass in the convolution blocks 
        return self.__conv_blocks(seq).squeeze(dim=1) # (BATCH SIZE, 5, SEQ LENGTH) -> (BATCH SIZE, SEQ LENGTH/8)
...
class PNASCNN(nn.Module):
    ...
    !Padding values provided were chosen to replicate the behavior obtained using 'padding=same' with Keras!
    ...
    def __init__(self, regression: bool = False) -> None:
        ...
        regression (bool): if True, no sigmoid activation function will be used at the end of the network.
        """
        super().__init__()
        
        # Block 1
        self.__block_1_0 = nn.Sequential(nn.Conv2d(1, 64, kernel_size=(8,5), padding='valid'),
                                         nn.ReLU())
        
        self.__block_1_1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=(8,1), padding='valid'),
                                         nn.ReLU(),
                                         nn.MaxPool2d(kernel_size=(8,1), stride=(8,1), padding=(2, 0)),
                                         nn.Dropout2d(0.25))
        
        # Block 2
        self.__block_2_0 = nn.Sequential(nn.Conv2d(64, 128, kernel_size=(8,1), padding='valid'),
                                         nn.ReLU())
        
        self.__block_2_1 = nn.Sequential(nn.Conv2d(128, 128, kernel_size=(8,1), padding='valid'),
                                         nn.ReLU(),
                                         nn.MaxPool2d(kernel_size=(8,1), stride=(8,1), padding=(1, 0)),
                                         nn.Dropout2d(0.25))
        
        # Block 3
        self.__block_3_0 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=(8,1), padding='valid'),
                                         nn.ReLU())
        
        self.__block_3_1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=(8,1), padding='valid'),
                                         nn.ReLU(),
                                         nn.MaxPool2d(kernel_size=(8,1), stride=(8,1), padding=(1, 0)),
                                         nn.Dropout2d(0.25))
    ...
    def forward(self,
        ...
        Args:
            seq_a (tensor): batch of DNA sequences (BATCH SIZE, 1, 3000, 5)
            seq_a (tensor): batch of DNA sequences (BATCH SIZE, 1, 3000, 5)
        ...
        x = cat([seq_a, zeros(seq_a.shape[0], 1, 30, 5).to(seq_a.device), seq_b], dim=2) # (BATCH SIZE, 1, 3000, 5), (BATCH SIZE, 1, 3000, 5) -> (BATCH SIZE, 1, 6030, 5)
        
        # Forward pass through block 1
        x = self.__block_1_1(nn.functional.pad(self.__block_1_0(x), (0, 0, 3, 4))) # (BATCH SIZE, 1, 6030, 5) -> (BATCH SIZE, 64, 750, 1)
        
        # Forward pass through block 2
        x = self.__block_2_0(nn.functional.pad(x, (0, 0, 3, 4))) # (BATCH SIZE, 64, 750, 1) -> (BATCH SIZE, 128, 750, 1)
        x = self.__block_2_1(nn.functional.pad(x, (0, 0, 3, 4))) # (BATCH SIZE, 128, 750, 1) -> (BATCH SIZE, 128, 94, 1)
        
        # Forward pass through block 3
        x = self.__block_3_0(nn.functional.pad(x, (0, 0, 3, 4))) # (BATCH SIZE, 128, 94, 1) -> (BATCH SIZE, 64, 94, 1)
        x = self.__block_3_1(nn.functional.pad(x, (0, 0, 3, 4))) # (BATCH SIZE, 64, 94, 1) -> (BATCH SIZE, 64, 12, 1)
