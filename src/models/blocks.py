...
from torch import abs, arange, cat, cos, exp, sign, sin, sqrt, stack, Tensor, where, zeros
from torch.nn import AvgPool1d, AvgPool2d, Conv1d, Conv2d, BatchNorm1d, BatchNorm2d, Dropout, Dropout1d, \
Dropout2d, Identity, LeakyReLU, Linear, MaxPool1d, MaxPool2d, Module, ModuleList, ReLU, Sequential
from torch.nn.functional import softmax


class TSSR(Module):
    """
    Activation function presented in "TSSR: A Truncated and Signed Square Root Activation Function for Neural Networks"
...
class LinearBlock(Module):
    ...
    def __init__(self, 
        ...
        Args:
            in_features (int): number of features in the input 2D tensor
            out_features (int): number of features in the output 2D tensor
            dropout (float): dropout probability
            bias (bool, optional): if True, bias is included in the linear layer.
            use_tanh_activation (bool, optional): if True, TanH activation is used instead of LeakyReLU.
        """
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Args:
            x (Tensor): Batch of features (N, IN FEATURES)
            
        Returns:
            Tensor: Batch of encoded features (N, OUT FEATURES)
        """
...
class DenseBlock1D(Module):
    ...
    def __init__(self,
        ...
        """
        Initializes the bottleneck and the convolution blocks.
        ...
        Args:
            ...
            dropout (float, optional): Dropout probability in dense layers. Default to 0.
            add_residual (bool, optional): if True, add residual connection between the input and the output of the dense block. Default to False.
            shrink_to_growth_rate (bool, optional): if True, apply bottleneck to the input so it has a number of channels equal to 'growth_rate'. Default to False.
        ...
        self.__add_residual = add_residual
        ...
        self.__conv_blocks = ModuleList([CNNBlock1D(in_channels=in_channels + growth_rate*i,
                                                    ...
                                                    stride=1,
                                                    padding=1,
                                                    dropout=dropout) for i in range(0, nb_conv_block)])
    def forward(self, x: Tensor) -> Tensor:
        """
        Executes the forward pass.
        ...
        Returns:
            Tensor: Encoded sequences (N, GROWTH RATE*NB CONV BLOCKS, LENGTH)
        """
...
class CNNBlock1D(Module):
    """
    Core block of the CNN 1D architecture.
    ...
    def __init__(self,
        ...
        Args:
            ...
            pool_size (int, optional): kernel size of pooling layer. If None, no pooling layer is included. Default to None.
            pooling_method (str, optional): pooling method ('max', 'avg' or 'attn'). Default to 'max.
            stride (int, optional): convolution stride. Default to 1.
            padding (int, optional): zero padding used during convolution. Default to 0.
            dropout (float, optional): Dropout probability. Default to 0.
            extended (bool optional): if True, the block is extended with additional layers in the middle. Default to False.
        """
        # Call parent's constructor
        Module.__init__(self)
        ...
        if pool_size is not None and (pooling_method not in ['max', 'avg', 'attn']):
            raise ValueError(f'Invalid pooling method: {pooling_method}. Must be "max", "avg" or "attn"')
        ...
        if pool_size is not None:
            if pooling_method == 'max':
                ...
                self.__pooling_layer = AttentionPool1D(in_channels=out_channels, kernel_size=pool_size)
        else:
...
class AttentionPool1D(Module):
    ...
    def __init__(self,
        ...
        """
        Initializes the weights of the convolution filters generating the unnormalized attention scores.
    ...
    def forward(self,  x: Tensor) -> Tensor:
        ...
        # (BATCH SIZE, NB CHANNELS, SEQ LENGTH) -> (BATCH SIZE, NB CHANNELS, SEQ LENGTH/POOL_SIZE, POOL_SIZE)
        x = x.reshape(x.shape[0], x.shape[1], -1, self.__kernel_size)
        
        # Calculate the attention
        # (BATCH SIZE, NB CHANNELS, SEQ LENGTH/POOL_SIZE, POOL_SIZE) -> (BATCH SIZE, 1, SEQ LENGTH/POOL_SIZE, POOL_SIZE)
        attn = softmax(self.__conv_layer(x), dim=-1) 
        
        # Apply attention pooling
        # (BATCH SIZE, NB CHANNELS, SEQ LENGTH/POOL_SIZE, POOL_SIZE) ->  (BATCH SIZE, NB CHANNELS, SEQ LENGTH/POOL_SIZE)
