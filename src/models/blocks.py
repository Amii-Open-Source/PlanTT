...
class MotifAttentionLayer(Module):
    """
    Attention layer dedicated to find important motifs within encoded DNA sequences.
    """
    def __init__(self,
        ...
        """
        Initializes the convolution layers and the attention mechanism.

        Args:
            max_motif_length (int, optional): maximum length of motif that can be identified.
                                              Default to 20.
            nb_channels (int, optional): nb of channels in the input sequences.
                                         Default to 5.
            embedding_size (int, optional): size of the motif embeddings generated.
                                            Default to 768.
        """
        # Initialize the convolution layers to capture k-mers
        self.__conv_layers = ModuleList([Conv1d(in_channels=nb_channels,
                                                out_channels=embedding_size,
                                                kernel_size=i,
                                                stride=1,
                                                padding=(0, i-1))
                                         for i in range(2, max_motif_length)])

        # Initialize the filter to calculate unnormalized attention
        self.__att_filter = Conv2d(in_channels=embedding_size,
                                   out_channels=1,
                                   kernel_size=1,
                                   stride=1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Generates embedded sequences that capture the essence of important motifs.

        Args:
            x (Tensor): batch of encoded nucleotide sequences (N, NB CHANNEL, LENGTH)

        Returns:
            Tensor: batch of sequences' embeddings (N, EMBED SIZE, LENGTH - 1)
        """

        # Extract the output of each convolution layer and stack them
        # (N, NB CHANNEL, LENGTH) -> (N, EMBED SIZE, MAX MOTIF LENGTH - 1, LENGTH - 1)
        kmers_embeddings = stack([layer(x).unsqueeze(dim=2) for layer in self.__conv_layers])

        # Calculate attention scores.
        # Scores are normalized on the height dimension (dim=2)
        # (N, EMBED SIZE, MAX MOTIF LENGTH - 1, LENGTH - 1) ->
        # (N, 1, MAX MOTIF LENGTH - 1, LENGTH - 1)
        att = softmax(self.__att_filter(x), dim=2)

        # Multiply each "pixel" by its attention score and sum on the height dimension (dim=2)
        # (N, EMBED SIZE, MAX MOTIF LENGTH - 1, LENGTH - 1) -> (N, EMBED SIZE, LENGTH - 1)
        return (kmers_embeddings * att).sum(dim=2)


class MultiHeadMAL(Module):
    """
    Multi-head motif attention layer.
    Combines the output of many motif attention layers.
    """
    def __init__(self,
                 nb_att_head: int,
                 max_motif_length: int = 20,
                 ...
                 embedding_size: int = 768):
        """
        Generates a list of motif attention layers and initializes a bottleneck layer
        to combine their output.
        ...
        Args:
            ...
            embedding_size (int, optional): size of the motif embeddings generated. 
                                            Default to 768.
        """
        # Initialize the motif attention layers
        self.__mals = ModuleList([MotifAttentionLayer(max_motif_length,
                                                     nb_channels,
                                                     embedding_size)
                                  for _ in range(nb_att_head)])

        # Initialize a convolution layer
        self.__bottleneck = Sequential(Conv1d(in_channels=embedding_size*nb_att_head,
                                             out_channels=embedding_size,
                                             kernel_size=1,
                                             stride=1),
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Concatenates the outputs of the motif attention layers and apply the bottleneck.
        ...
        return self.__bottleneck(stack([layer(x) for layer in self.__mals], dim=1))
