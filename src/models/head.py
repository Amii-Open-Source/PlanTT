...
class MLPHead(Head):
    ...
    def __init__(self,
        ...
        Args:
            regression (bool): if True, a sigmoid activation will be added
                               after the aggregation function.
            input_size (int): length of the embeddings provided as input.
            dropout (float, optional): dropout probabilities in the linear block. 
                                       Default to 0.
            odd (bool, optional): if True, the mlp will be an odd function. 
                                  Default to True.
        """
        super().__init__(regression)

        if odd:
            # Use odd activation function and remove bias from linear block
            ...
            # Use LeakyReLU activation function and include bias in linear block
            self.__layers = Sequential(LinearBlock(in_features=input_size,
                                                   out_features=50,
                                                   dropout=dropout,
                                                   bias=True,
...
class SumHead(Head):
    """
    PlanTT head that sums the elements in the vector storing the difference of the embeddings.
    ...
    def __init__(self, regression: bool) -> None:
        ...
        Args:
            regression (bool): if True, a sigmoid activation will be added
                               after that aggregation function.
        """
        super().__init__(regression)
