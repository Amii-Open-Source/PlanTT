...
Description: Stores the classes associated to tower architectures based on Masked Language Model (MLM).
"""
...
class dDNABERT(Tower):
    ...
    def __init__(self, freeze_method: str = None) -> None:
        ...
        # (we cannot assign a module to a private attribute yet because tower init need to be called first)
    ...
    def forward(self, seq: Tensor) -> Tensor:
        ...
        Since sequences are longer than 512 tokens, each of them is separated into multiple chunks of 512 tokens.
        ...
        seq = stack(self.__dnabert(seq).hidden_states, dim=2).mean(dim=2)  # (BATCH SIZE*NB CHUNK, 512) -> (BATCH SIZE*NB CHUNK, 512, 384)
        
        # Pass embeddings through the adapter layer
        return self.__adapter_layer(seq.permute(0, 2, 1)) # (BATCH SIZE, 512, 384) -> (BATCH SIZE, 512)
...
class DNABERT(Tower):
    ...
    def __init__(self,
        ...
        pre_trained_weights (str, optional): Path from which pre-trained weights of DNABERT-k must be loaded.
        """
        # Initialize the adapter layer
        # (we cannot assign a module to a private attribute yet because tower init need to be called first)
    ...
    def forward(self, seq: Tensor) -> Tensor:
        ...
        seq = stack(self.dna_bert(input_ids=seq, output_hidden_states=True).hidden_states, dim=2).mean(dim=2)  # (BATCH SIZE*NB CHUNK, 512) -> (BATCH SIZE*NB CHUNK, 512, 768)
        
        # Pass embeddings through the adapter layer
        return self.__adapter_layer(seq.permute(0, 2, 1)) # (BATCH SIZE* NB CHUNK, 512, 768) -> (BATCH SIZE, 768)
    
    
class MLMAdapter(Module):
    """
    Module in charge of encoding the embeddings generated by Masked Language Model (MLM) in a proper format for PlanTT.
    """
    def __init__(self,
                 nb_chunks: int = 6,
        ...
        Args:
            nb_chunks (int, optional): Number of chunks associated to each complete nucleotides sequences. Default to 6.
            chunk_length (int, optional): Length of each chunk. Default to 768.
            chunk_depth (int, optional): Number of channels into each chunk. Default to 512.
            kernel_size (int, optional): Kernel size used in the last convolution layer. Default to 6.
            stride (int, optional): Stride used in the last convolution layer. Default to 6.
        ...
        self.__bottleneck = CNNBlock1D(in_channels=chunk_depth, out_channels=1, kernel_size=1, stride=1)
        self.__out_size: float = ((nb_chunks*chunk_length) - kernel_size)/stride + 1
        
        if not self.__out_size.is_integer():
            raise ValueError('Input values must be change to ensure that ((NB CHUNKS*CHUNK LENGTH) - KERNEL SIZE)/STRIDE is an integer')
        else:
            self.__out_size = int(self.__out_size)
        
        self.__conv_layer = CNNBlock1D(in_channels=1, out_channels=1, kernel_size=kernel_size, stride=stride)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Args:
            x (Tensor): Nucleotides sequence embeddings (BATCH SIZE*NB CHUNKS, CHUNK DEPTH, CHUNK LENGTH)

        Returns:
            Tensor: Nucleotides sequences's final embeddings (BATCH SIZE, OUT SIZE)
        """
        
        # Apply a bottle neck that summarizes all tokens in the sequence
        x = self.__bottleneck(x).squeeze(dim=1)  # (BATCH SIZE*NB CHUNKS, CHUNK DEPTH, CHUNK LENGTH) -> (BATCH SIZE*NB CHUNKS, CHUNK LENGTH)

        # Reshape the tensor to concatenate chunks associated to each data point (i.e. each sequence)
        x = x.reshape(-1, 1, self.__chunk_length*self.__nb_chunks) # (BATCH SIZE*NB CHUNKS, CHUNK LENGTH) -> (BATCH SIZE, 1, CHUNK LENGTH*NB CHUNKS)

        # Apply convolution on concatenated chunks
        x = self.__conv_layer(x).squeeze(dim=1)  # (BATCH SIZE, 1, CHUNK LENGTH*NB CHUNKS) -> (BATCH SIZE, OUT SIZE)
